...
if __name__ == '__main__' :
    ...
    with tf.name_scope('model') :
        ...
        with tf.name_scope('compute_loss'):
            # 定义交叉熵
            loss_op = -tf.reduce_sum(y * tf.log(tf.clip_by_value(y_pred, 1e-10, 1.0)), name='loss_op')
            # 定义MSE
            # loss_op = tf.reduce_sum(tf.square(y_pred - y), name='loss_op')
            # 监控交叉熵
            loss_scalar = tf.summary.scalar('loss', loss_op)
            # 定义优化器和训练器
            train_step = tf.train.AdamOptimizer(1e-4).minimize(loss_op, global_step=global_step)
    ...
    with sv.managed_session(config=config) as sess :
        ...
        while not sv.should_stop():
            ...
            if step%ITERATION_SHOW == 0:
                ...
                # 计算当前训练集的准确率
                merged_summary, accuracy, loss = sess.run([merged_summary_op, accuracy_op, loss_op], feed_dict={x: batch[0], y: batch[1], keep_prob: 1.0})
                ...
                (accuracy_validation_sum,
                 loss_validation_sum,
                 accuracy_validation, loss_validation) = sess.run([accuracy_scalar, loss_scalar, accuracy_op, loss_op], feed_dict={x:mnist.validation.images, y: mnist.validation.labels, keep_prob: 1.0})
                # write to summary
